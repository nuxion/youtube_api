...
class VideoItem(BaseModel):
    ...
    tags: List[str]
    published_at: str = Field(alias="publishedAt")
    default_audio_lang: str = Field(alias="defaultAudioLanguage")
    default_lang: str = Field(alias="defaultLanguage")
    title: str
    thumbnail: str
    channel_id: str = Field(alias="channelId")
    channel_title: str = Field(alias="channelTitle")
...
class Client:
    ...
    def __init__(self, api_key=os.getenv("API_KEY"), debug=False):
        ...
        self._debug = debug
        self.c = httpx.Client(headers=self._h)
        self._raw_rsp = None
    ...
    def _item2search_item(self, item: Dict[str, Any]) -> SearchItem:
    ...
    def _search_result2pydantic(self, rsp: Dict[str, Any]) -> SearchResponse:
        items = [self._item2search_item(i) for i in rsp["items"]]
    ...
    def search(self, q: SearchQuery) -> SearchResponse:
        ...
        uri = urlencode(data)
        rsp = self.c.get(f"{self.URL}/search?{uri}")
        self._store_rsp(rsp)

        return self._search_result2pydantic(rsp.json())

    def video(self, v: VideoQuery):
        data = v.dict(by_alias=True, exclude_none=True)
        data.update({"key": self._api_key})
        uri = urlencode(data)
        rsp = self.c.get(f"{self.URL}/videos?{uri}")
        self._store_rsp(rsp)
    ...
    def _dict2videoitem(self, item: Dict[str, Any]) -> VideoItem:
        ...
        return VideoItem(
            ...
            title=s["title"],
            thumbnail=s["thumbnails"]["default"]["url"],
            default_audio_lang=s["defaultAudioLanguage"],
            default_lang=s["defaultLanguage"],
            channel_id=s["channelId"],
            channel_title=s["channelTitle"],
